...
const char* menuItems[] = {
  "Relogio","Calendario","Temperatura",
  "Cronometro","Timer","Bateria",
  "Sistema","Snake","Config"
};
...
void drawTimer(){
  if(!tRun){
    if(bUp.fell()) tMin++;
    if(bDown.fell()) tMin = max(1, tMin-1);
  }
  if(bOk.fell()){
    tRun = !tRun;
  ...
  unsigned long elapsed = tRun ? millis()-tStart : 0;
  unsigned long rem = elapsed >= total ? 0 : total - elapsed;
...
/* ================= SNAKE ================= */
struct P { int x,y; } snake[32], food;
...
void drawSettings(){
  if(bUp.fell()||bDown.fell()) setIdx = !setIdx;
  ...
  display.setCursor(0,0);
  display.printf("%cBrilho:%d", setIdx==0?'>':' ', settings.brightness);
  display.setCursor(0,12);
  display.printf("%cSom:%s", setIdx==1?'>':' ', settings.sound?"ON":"OFF");
}
